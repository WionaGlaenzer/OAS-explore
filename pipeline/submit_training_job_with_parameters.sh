...
export PROJECT_BASE="/cluster/project/reddy/wglaenzer/final_training_testing_val_data/OAS-wo-Soto"
...
export WANDB_DIR=/cluster/project/reddy/wglaenzer/wandb_logs
# Directory for caching artifacts, etc. (use scratch space)
export WANDB_CACHE_DIR=/cluster/scratch/wglaenzer/wandb_cache
# Optional: Directory for config files (use project space)
export WANDB_CONFIG_DIR=/cluster/project/reddy/wglaenzer/.config/wandb
export TORCH_EXTENSIONS_DIR=/cluster/scratch/wglaenzer/caches/torch_extensions
...
sbatch -A es_reddy --job-name="${JOB_NAME}" \
    ...
    \"${DONE_FILE_PATH}\" \
    \"${TOKENIZER_PATH}\" \
    "/cluster/home/wglaenzer/Coding/plm_training_pipeline/assets/deepspeed_stage2_config.json" \
